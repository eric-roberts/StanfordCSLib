...
static void testStreamScanner(void) {
   ...
   test(nextToken(scanner), " ");
   test(hasMoreTokens(scanner), true);
   test(nextToken(scanner), "Makefile");
   test(hasMoreTokens(scanner), true);
   test(nextToken(scanner), " ");
   test(nextToken(scanner), "for");
   test(nextToken(scanner), " ");
   test(nextToken(scanner), "StanfordCSLib");
   trace(ignoreWhitespace(scanner));
   trace(scanStrings(scanner));
   test(nextToken(scanner), "directory");
   trace(fclose(infile));
}
